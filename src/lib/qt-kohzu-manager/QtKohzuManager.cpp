...
void QtKohzuManager::startMonitoring(const std::vector<int>& axes)
{
    ...
    // This is a simplified approach. A better way would be to get updates from the controller.
    // For now, we use AxisState's cache.
    QTimer *timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, [this, axes](){
        for(int axis : axes) {
...
void QtKohzuManager::stopMonitoring()
{
    ...
    emit logMessage("Stopped monitoring.");
}
...
void QtKohzuManager::move(int axis, int value, bool isAbsolute)
{
    ...
    if (isAbsolute) {
        ...
        controller_->moveAbsolute(axis, value, speed, responseMethod, callback);
    } else {
        emit logMessage(QString("Moving axis %1 by relative distance %2...").arg(axis).arg(value));
        controller_->moveRelative(axis, value, speed, responseMethod, callback);
    }
}
